...
import {getcompaniesByTitle} from '../../../../api/titleSubtitle';
...
const SpecificTitleCompany = ({route}) => {
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({
      title: company.title.text || 'Category Name',
    });
  }, [navigation, company.title.text]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const title = company.title.text;
        const token = await AsyncStorage.getItem('token');
        showLoader();
        const response = await getcompaniesByTitle(token, title);
...
export default SpecificTitleCompany;
